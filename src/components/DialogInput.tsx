...
const StyledInput = styled(InputBase)`
  ...
  & .MuiInputBase-input {
    ...
    background-color: ${({ theme }) =>
      (theme as Theme).palette.mode === 'light' ? '#F3F6F9' : '#1A2027'};
    border: 1px solid
      ${({ theme }) =>
        (theme as Theme).palette.mode === 'light' ? '#E0E3E7' : '#2D3843'};
    font-size: 16px;
    width: auto;
    padding: 10px 12px;
    transition: ${({ theme }) =>
      (theme as Theme).transitions.create([
        'border-color',
        'background-color',
        'box-shadow',
      ])};
  ...
  &:focus {
    box-shadow: ${({ theme }) =>
      `${alpha((theme as Theme).palette.primary.main, 0.25)} 0 0 0 0.2rem}`};
